...
| AI activity stream (per-user tool inputs, commands, file paths) — **opt-in** | CloudWatch `/claude/<prefix>/activity` → Kinesis Firehose → S3 archive bucket (`activity/` prefix) | CloudWatch window **14 days** *(param `ACTIVITY_LOG_WINDOW_DAYS`)*; durable S3 archive **731 days / 2 years** *(param `ACTIVITY_ARCHIVE_RETENTION_DAYS`)* | **Highly sensitive** — IAM-only access, CMK-encrypted, flagged for SIEM. Prompt and response content is excluded by default; the separate independent opt-ins `LOG_USER_PROMPTS=true` and `LOG_ASSISTANT_RESPONSES=true` (stack 02) add the full user-typed prompt text and the model's response text respectively — same destinations and retention, sharply higher sensitivity. Archive delivery buffers up to 5 minutes. S3 Object Lock was evaluated and deferred by decision (tracked in the security review). |
...
| Load-balancer access log (every HTTPS request, source IP, path) | S3 ALB-logs bucket | **90 days** *(param `ALB_LOG_RETENTION_DAYS`)* | SSE-S3 encryption, not the CMK — ELB log delivery does not support KMS (the one deliberate exception; do not "fix"). |
