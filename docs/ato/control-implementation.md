...
| AU-12 | Implemented | Generation is on by default for pgaudit, ALB, portal and spend-cap audit; the AI activity stream is opt-in (`FORWARD_ACTIVITY_LOGS`), prompt-content capture within it is a second opt-in (`LOG_USER_PROMPTS`, refused without the first), and Bedrock prompt logging is opt-in and tri-state (`BEDROCK_PROMPT_LOGGING`). | `cloudformation/03-observability.yaml`; `scripts/deploy-observability.sh`; `cloudformation/02-gateway.yaml` (`LogUserPrompts`) | `conops.md` §5.4; `om-runbooks.md` §11 |
