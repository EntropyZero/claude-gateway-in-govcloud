...
| IT-5 | **AI activity records (opt-in)** | Per-user bash commands, tool inputs and file paths. Prompt content is redacted from this stream by default; the separate opt-in `LOG_USER_PROMPTS=true` adds the full user-typed prompt text (per-user attributed, this gateway's clients only, no model responses) | CloudWatch `/claude/<prefix>/activity` (14 d window) → Firehose → S3 archive (731 d), both CMK | Only when `FORWARD_ACTIVITY_LOGS=true`; bursty, tied to real tool use |
...
- **IT-5's redaction matters to the rating.** Prompt content is redacted from
  the activity stream by default, which is why IT-5 sits at Moderate while
  IT-7 sits at High despite both concerning user content. Enabling
  `LOG_USER_PROMPTS=true` removes that redaction: IT-5 then carries verbatim
  user-typed prompt text with per-user attribution, and by this document's
  own logic its confidentiality impact rises to that of IT-7 (see the
  configuration table below). Like IT-7, it is a categorization-relevant
...
| Activity stream **with prompt capture** (`LOG_USER_PROMPTS=true`) | **High** | Moderate | Low | **High** |
