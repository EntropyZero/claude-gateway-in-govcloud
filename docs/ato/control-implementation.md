...
| AC-20 | Implemented | Client-side external system use is constrained by the managed catch-all policy: `WebSearch` and `mcp__*` are denied as bare tool names, so they are removed from the model's context and cannot be re-allowed at user scope. `WebFetch` was removed from the deny list by decision (2026-08-07): it fetches locally on the client, bounded by the same Zscaler client-side web policy as the developer's browser — the same boundary already accepted for `Bash` (`curl`/`wget`). | `cloudformation/02-gateway.yaml` (`GATEWAY_MANAGED_B64`, `cli.permissions.deny`) | `client-config.md` §6; accepted risk A8 (`Bash`, `Agent`, and — since 2026-08-07 — `WebFetch` deliberately not denied) |
...
| AU-12 | Implemented | Generation is on by default for pgaudit, ALB, portal and spend-cap audit; the AI activity stream is opt-in (`FORWARD_ACTIVITY_LOGS`), prompt and response content capture within it are further independent opt-ins (`LOG_USER_PROMPTS` / `LOG_ASSISTANT_RESPONSES`, each refused without the first), and Bedrock prompt logging is opt-in and tri-state (`BEDROCK_PROMPT_LOGGING`). | `cloudformation/03-observability.yaml`; `scripts/deploy-observability.sh`; `cloudformation/02-gateway.yaml` (`LogUserPrompts`, `LogAssistantResponses`) | `conops.md` §5.4; `om-runbooks.md` §11 |
