...
**[docs/ato/architecture.md](docs/ato/architecture.md)**.

## Layout

| Path | Purpose |
|---|---|
| [`docs/ato/architecture.md`](docs/ato/architecture.md) | Review package: architecture, data-flow, credential & network diagrams (SVGs in `docs/ato/diagrams/`, generated by `generate.py`) |
...
| `scripts/mirror/mirror-collector.sh` | Mirror a pinned ADOT collector into ECR; persists `COLLECTOR_IMAGE` |
...
`scripts/mirror/mirror-claude-release.sh <version>` (downloads, checksum-verifies, and
...
developer **with local admin**) — see `docs/operations/client-config.md` for both
mechanisms and `docs/requests/ad-request-email.md` for the AD/GPO request. Once the
...
see `docs/operations/client-config.md` for both mechanisms (GPP Registry to
`HKLM\SOFTWARE\Policies\ClaudeCode`, or `managed-settings.json` to
`%ProgramFiles%\ClaudeCode\`) and `docs/requests/ad-request-email.md` for the AD/GPO
...
segment. If you use the GPO path for forced login (`docs/operations/client-config.md`),
...
ADOT_VERSION=v0.43.0 ./scripts/mirror/mirror-collector.sh  # mirrors ADOT → ECR; writes COLLECTOR_IMAGE (the gateway-task sidecar image)
