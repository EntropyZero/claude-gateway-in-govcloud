...
| Activity stream (opt-in) | bash commands, tool inputs, file paths, per user; prompt content redacted by default (full prompt text added only with the separate opt-in `LOG_USER_PROMPTS=true`) | CloudWatch (CMK) → S3 (SSE-KMS) | 14 d window / 731 d archive | IAM only; flagged for SIEM subscription |
